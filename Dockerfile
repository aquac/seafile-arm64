...
FROM phusion/baseimage:18.04-1.0.0-arm
ENV SEAFILE_SERVER=seafile-server SEAFILE_VERSION=7.1.4
...
RUN mkdir -p /opt/seafile/ && cd /opt/seafile/ && \
    wget https://github.com/haiwen/seafile-rpi/releases/download/v${SEAFILE_VERSION}/seafile-server_${SEAFILE_VERSION}_pi-bionic-stable.tar.gz && \
